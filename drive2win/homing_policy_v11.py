...
v11: Longer reverse (2 seconds), forced reset detection.
"""
...
STEER_GAIN       = 1.4
STUCK_THRESHOLD  = 15
REVERSE_FRAMES   = 40      # 2 seconds at 20Hz (was 10)
STUCK_SPEED      = 0.3
RAY_WEDGE        = 4.0
PURE_STUCK_THR   = 50
...
# Forced reset detection
RESET_THRESHOLD  = 120    # 6 seconds stuck in same spot
RESET_FRAMES     = 20     # Hold reset for 1 second
...
def make_policy(weights_path: str):
    ...
    def policy(state: dict) -> tuple[float, float]:
        ...
        nonlocal reset_count, last_positions, last_cp, cp_stuck_frames
        ...
        current_cp = sensors.get("navigation", {}).get("checkpoints_completed", 0)
        
        # Track position for stuck detection
        pos = state.get("position")
        if pos:
            current_pos = (pos.get("x", 0), pos.get("z", 0))
            last_positions.append(current_pos)
            if len(last_positions) > 30:
                last_positions.pop(0)

        # Check if checkpoint progress is stuck
        if current_cp == last_cp and cp_dist < 15.0 and len(last_positions) >= 30:
            # Check if position hasn't changed much
            start_pos = last_positions[0]
            end_pos = last_positions[-1]
            ...
            if moved < 1.0:
                ...
                cp_stuck_frames = max(0, cp_stuck_frames - 5)
        else:
            cp_stuck_frames = max(0, cp_stuck_frames - 2)
        last_cp = current_cp

        # Forced reset if truly stuck
        if cp_stuck_frames >= RESET_THRESHOLD:
            if reset_count == 0:
                print("  FORCED RESET - car stuck")
            reset_count += 1
            if reset_count <= RESET_FRAMES:
                # Send reset via R key (throttle=steering=99 signals reset in eval.py)
                return (99.0, 99.0)  # Special value for reset
            else:
                reset_count = 0
                cp_stuck_frames = 0
        ...
        # Orbit escape
        cp_near = cp_dist < CP_ORBIT_DIST
        if cp_near:
            orbit_frames += 1
        else:
            orbit_frames = max(0, orbit_frames - 2)
        ...
        if reverse_count > 0:
            reverse_count -= 1
            # Alternate steering while reversing
            if reverse_count % 10 < 5:
                steer = -0.9 if right < left else 0.9
            else:
                steer = 0.0
